...
mod tests {
    ...
    #[test]
    fn discourse_openers_are_not_names_and_more_hedges_are_hedges() {
        assert_eq!(
            anchors_of("Therefore, based on the memories, I cannot confirm it."),
            Vec::<String>::new()
        );
        assert!(is_hedge(
            "There is no direct mention of a watchdog in the memories."
        ));
        assert!(is_hedge("The specific watchdog task is not among them."));
    }
